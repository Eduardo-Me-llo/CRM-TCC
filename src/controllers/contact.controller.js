...
const { validateEmail, validatePhone } = require('../utils/normalizers');
...
async function create(req, res) {
  ...
  if (email && !validateEmail(email)) return res.status(400).json({ message: 'E-mail inválido.' });
  if (phone && !validatePhone(phone)) return res.status(400).json({ message: 'Telefone inválido.' });
  if (whatsapp && !validatePhone(whatsapp)) return res.status(400).json({ message: 'WhatsApp inválido.' });
